...
while($donnees = mysqli_fetch_assoc($annees))
{    // $pres='chiffres';
			$oy=date('Y')-1;
			...
			if($donnees['annee']<$oy){$pres='Chiffres';} else {$pres='Prévisions';}
				echo'<div class="da-slide">
					...
					<div class="da-img">
            <img src="pages/image.php?annee='.$donnees['annee'].'" alt="Chiffres de '.$donnees['annee'].'" /></div>
				</div>';
				}
      mysqli_free_result($annees);
