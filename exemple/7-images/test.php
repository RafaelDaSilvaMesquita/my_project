    <?php
    // // on définit un tableau contenant le nombre de page vues par mois : par exemple, on suppose que 1500 pages du site ont �t� vues en janvier, 2450 en f�vrier, etc... Bien sur, pour que ce script soit vraiment valides, vous n'allez pas d�clarer ce tableau, car sinon, les b�tons seront toujours les m�mes :) Vous allez plut�t effectuer une requ�te SQL sur votre base de donn�es permettant de r�cup�rer le nombre de pages vues de votre site par mois. Ensuite, il suffira d'appeler le script avec ces 12 param�tres dans votre page html (en faisant par exemple : <img src="./images/stats_year.php?visite_par_mois[1]=1500.......">)
    ...
    // $font = 'Carre.ttf';
    // //echo $font;
    // // on crée une ressource pour notre image qui aura comme largeur $largeur et $hauteur comme hauteur (on place �galement un or die si la cr�ation se passait mal afin d'avoir un petit message d'alerte)
    ...
    // // on place tout d'abord la couleur blanche dans notre table des couleurs (je vous rappelle donc que le blanc sera notre couleur de fond pour cette image).
     $blanc = ImageColorAllocate ($im, 255, 255, 200);
    ...
    // // on affiche le dernier baton, bleu clair, qui sera un peu plus petit que le bleu fonc� afin de recouvrir une partie du bleu fonc� (on obtiendra ainsi un effet de d�grad�)
